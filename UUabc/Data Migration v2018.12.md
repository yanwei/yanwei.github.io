...
* 老系统中的所有课时都不直接迁移到2.0系统，只能在老系统中消耗，或者按一定比例转换成U币。
* 1v4课时可以按补足原则，转换成对应的四季切齐课程包。如：用户账户剩余30个1v4课时，可以再购买36个课时（30 + 36 = 66），直接转换成四季切齐寒春连报课程包。
* 因此，从老系统到2.0系统的迁移，只涉及四季切齐课程包和U币。
* 迁移过程必须在新老系统做好所有相关数据记录。

|资产类型|处理原则|迁移方式|
|---|---|---|
|1v1课时|可在1.0系统中继续消耗，不直接迁移到2.0系统|可按一定比例转换成U币|
|1v4课时|可在1.5系统中继续消耗，不直接迁移到2.0系统，补齐课时后可转换成四季切齐课程包|可按一定比例转换成U币|
|直播课时|不直接迁移到2.0系统|可按一定比例转换成U币|
|U币|可按1:1比例迁移到2.0系统|直接迁移|
|积分/等级|不迁移|-|
|钻石|不迁移|-|
|课时明细|不迁移|-|
|上课记录、作业和评语|不迁移|-|
...
* Case 2: 学生课时不足，补足后迁移到2.0系统，迁移后1.5系统没有剩余课时。
* Case 3: 学生课时足够，迁移到2.0系统后，课时依然有剩余，按一定兑换比例转换成U币。
